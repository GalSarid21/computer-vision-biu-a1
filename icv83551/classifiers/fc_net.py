...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        scores, fc_cache2 = affine_forward(h1, W2, b2)
        ...
        reg_elems_factor = 0.5
        reg_loss = reg_elems_factor * self.reg * (np.sum(W1 * W1) + np.sum(W2 * W2))
...
class FullyConnectedNet(object):
    ...
    def __init__(
        ...
        for i in range(1, self.num_layers + 1):
            w_key = f"W{i}"
            b_key = f"b{i}"
            self.params[w_key] = weight_scale * np.random.randn(dims[i-1], dims[i])
            self.params[b_key] = np.zeros(dims[i], dtype=dtype)

            # gamma/beta only for hidden layers (1..L-1), if normalization is used
            if i != self.num_layers and self.normalization in ("batchnorm", "layernorm"):
                self.params[f"gamma{i}"] = np.ones(dims[i], dtype=dtype)
                self.params[f"beta{i}"]  = np.zeros(dims[i], dtype=dtype)
    ...
    def loss(self, X, y=None):
        ...
        loss_elems_factor = 0.5
        for i in range(1, self.num_layers + 1):
            W = self.params[f"W{i}"]
            loss += loss_elems_factor * self.reg * np.sum(W * W)
        ...
        for i in range(self.num_layers - 1, 0, -1):
            ...
            if self.normalization == "batchnorm":
              ...
              print("[warning] no normalization method in use.")
